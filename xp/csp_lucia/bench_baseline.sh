#!/bin/bash
#
#SBATCH --job-name=csp_bench
#SBATCH --output=logs/csp_bench_%A_%a.out  # stdout log for each array task
#SBATCH --error=logs/csp_bench_%A_%a.err   # stderr log for each array task
#
...
INPUT_FILE="${PROJECT_DIR}/examples/lucia_input/input_lucia_csp_baseline.txt"
