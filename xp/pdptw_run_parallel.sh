...
nParallel=40  # number of parallel run (should be <= number of threads on the machine, but small enough to fit in memory)
run_script="${PREFIX}/xp/pdptw_run_one_instance.sh"  # executable for running the experiments
...
instances="${PREFIX}/examples/pdptw/test_instances_2023.txt"


. "${PREFIX}/setup.sh"
myDate=`printf '%(%Y-%m-%d_%H_%M_%S)T\n' -1`
commitId=`git rev-parse --short HEAD`
outFilename="${PREFIX}/xp/pdptw_${myDate}_${commitId}_results.csv"  # where the results will be written
...
echo "instance,bandit,timeout,unroutedNodes,nVehicles,travelLength,objective,integralPrimalGap,solOverTime" > $outFilename

for (( i=1; i<=$nRuns; i++ ))  # one line per solver
do
  for bandit in "${BanditType[@]}"
  do
    # extracts the instances from the data folder, prepending PREFIX to each line
    cat $instances | sed "s|^|${PREFIX}|; s/$/,$bandit/"  >> $inputFile
  done
done
