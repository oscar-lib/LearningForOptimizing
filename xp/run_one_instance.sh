...
output=`$launch_solver --input ${instance} --timeout ${timeout} --bandit ${bandit} --verbosity 1`
...
objective=$(echo "$output" | grep 'Total route length' | awk -F': ' '{print $2}')
# prints the relevant informations
echo "$instance,$bandit,$timeout,$unroutedNodes,$nVehicles,$objective "