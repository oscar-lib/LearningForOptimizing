...
run_script="./xp/tsp_run_one_instance.sh"  # executable for running the experiments
# path to the file where the instances to run are written
instances="examples/tsp/tsplib"

myDate=`printf '%(%Y-%m-%d_%H_%M_%S)T\n' -1`
commitId=`git rev-parse --short HEAD`
outFilename="results/tsp_${myDate}_${commitId}_results.csv"  # where the results will be written

# ------ compilation + input preparation -------
inputFile="tsp_parallel_input"  # where the input data will be written for this experiment
...
echo "instance,bandit,timeout,unroutedNodes,travelLength,objective,solOverTime,integralPrimalGap" > $outFilename
...
do
  ...
  do
    ...
    find $instances -type f | sed "s/$/,$bandit/"  >> $inputFile
  done
done
