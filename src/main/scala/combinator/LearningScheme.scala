 ...
 * @param n number of consecutive moves that must be performed before learning
 */
case class AfterNMoves(n: Int) extends LearningScheme {
  ...
  /**
   * Gives the current consecutive number of moves performed since the reset of the learning scheme.
   */
  def nMoves = _nMoves
  ...
  /**
   * Tells if the number of consecutive moves has been reached.
   * The counter should be reset after having learnt
   */
