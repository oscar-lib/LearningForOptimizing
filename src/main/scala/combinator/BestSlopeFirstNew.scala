...
class BestSlopeFirstNew(neighborhoods: List[Neighborhood]) extends BanditSelector(neighborhoods, AfterEveryMove) {
   ...
   */
  override def newWeightFromReward(neighborhood: Neighborhood, oldWeight: Double, reward: Double): Double = {
