...
case class SimpleNeighborhoods(
  ...
  private def randVehicle(): () => Int = { () =>
    Random
      .shuffle(
        Array.tabulate(pdptw.v)(x => x).filter(x => pdptw.getRouteOfVehicle(x).drop(1).nonEmpty)
      )
      .head
  }
  ...
  def pickupInsertUnroutedFirst(
    ...
    InsertPointUnroutedFirst(
      ...
      relevantPredecessor = () =>
        node =>
          pdptw
            .kFirst(k, closestRelevantPredecessors(_), _ => node => pdptw.isRouted(node))(node)
            .toList ::: pdptw.vehicles.toList,
      vrp = pdptw,
  ...
  /** Generates a Neighborhood whose purpose is to insert a new node in the route.
    *
    * It first iterate over the position of insertion and then iterate over the point to insert. Let
    * say we have the points (4,5,6,7) to insert and the available position are after points
    * (0,1,2,3,8,9,10) It'll take the first insertion position, 0 and try to insert 4 after it, then
    * 5, 6... If no solution where found it'll try with the position 1, then 2, 3...
    *
    * @param k
    *   The maximum number of position to test
    * @param insertAfter
    *   A function returning the list of insertion position.
    * @param hotRestart
    *   If the algorithm restarts where it finished last time. Lets say we inserted after point 2.
    *   If true it will try to insert after point 3 at the next round if false, it'll restart try
    *   after point 0.
    * @param best
    *   Test all the possible insertions and keep the best one. Take much more time.
    * @return
      ...
      relevantSuccessorsToInsert = () =>
        node =>
          pdptw
            .kFirst(
              k,
              closestRelevantSuccessors(_),
              _ => node => pdptw.isUnrouted(node) && oscarModel.isPickupPoint(node)
            )(node)
            .toList,
      vrp = pdptw,
      ...
      () =>
        pdptw.kFirst(
          ...
          ChainsHelper.relevantNeighborsForLastNodeAfterHead(
            pdptw,
            oscarModel.chains,
            Some(HashSet() ++ closestRelevantPredecessors(deliveryToInsert))
          ),
          _ => node => true
        ),
      vrp = pdptw,
  ...
  /** Moves one point to another position
    *
    * @param k
    *   The maximum number of positions to explore
    * @param pointsToMove
    *   A function returning the list of point to move. If not specified it'll take the routed
    *   points. Mainly used for one couple move (see below)
    * @param hotRestart
    *   If the algorithm restarts where it finished last time.
    * @param best
    *   Test all the possible moves and keep the best one. Take much more time.
    * @return
    *   The neighborhood that includes the above specifications
    */
  def onePointMove(
    ...
    OnePointMove(
      ...
      relevantNewPredecessors =
        () => pdptw.kFirst(k, closestRelevantPredecessors(_), _ => node => pdptw.isRouted(node)),
      vrp = pdptw,
      hotRestart = hotRestart,
      ...
      relevantNewPredecessors = () =>
        pdptw.kFirst(
          ...
          ChainsHelper.relevantNeighborsForLastNodeAfterHead(
            pdptw,
            oscarModel.chains,
            Some(HashSet() ++ closestRelevantPredecessors(delivery))
          ),
  ...
  /** Removes one point from the route. Useful to get out of local minima.
    ...
    *   A function returning the list of points to remove. If not specified, it'll take the routed
    *   points.
    * @return
    *   A neighborhood that removes points from the route
    */
  ...
  def couplePointInsertUnroutedFirst(
    k: Int,
    ...
    dynAndThen(
      pickupInsertUnroutedFirst(k, pickUpPointsToInsert, hotRestart, best),
      (move: InsertPointMove) => {
  ...
  def couplePointInsertRoutedFirst(
    k: Int,
    ...
    dynAndThen(
      pickupInsertRoutedFirst(k, insertAfterPoints, hotRestart, best),
      (move: InsertPointMove) => {
        val deliveryPoint = oscarModel.pickupPointToDeliveryPoint(move.insertedPoint)
  ...
  def couplePointMove(
    ...
    dynAndThen(
      onePointMove(k, pickUpPointsToMove, hotRestart, best),
  ...
  def couplePointRemove(pickUpPointsToRemove: () => Iterable[Int] = routedPickups): Neighborhood = {
    dynAndThen(
      onePointRemove(() => pickUpPointsToRemove()),
      (move: RemovePointMove) => {
  ...
  def emptyVehicle(vehicle: () => Int = randVehicle()): Neighborhood = {
    ...
    atomic(
      dynAndThen(
        onePointRemove(() => pickUpsToRemove),
        (move: RemovePointMove) => {
          val deliveryPoint = oscarModel.pickupPointToDeliveryPoint(move.pointToRemove)
          onePointRemove(() => List(deliveryPoint))
        }
      ).acceptAll(),
      _ > nbOfPickupToRemove
    ) onQuery {
      ...
      (move: CompositeMove) => {
        ...
        if (!pdptw.movingVehicles.exists(v => v == vehicleTo))
          ...
          dynAndThen(
            onePointMove(
              k,
              () => pdptw.getRouteOfVehicle(vehicleTo).filter(oscarModel.isPickupPoint),
              hotRestart,
              best
            ),
            (move: OnePointMoveMove) => {
    ...
    dynAndThen(
      ...
      (move: CompositeMove) => {
        ...
        if (!pdptw.movingVehicles.exists(v => v == vehicleTo))
          ...
          dynAndThen(
            pickupInsertRoutedFirst(k, () => pdptw.getRouteOfVehicle(vehicleTo)),
            (move: InsertPointMove) => {
              val deliveryPoint = oscarModel.pickupPointToDeliveryPoint(move.insertedPoint)
  ...
  def segmentExchanges(k: Int, best: Boolean = false): Neighborhood = {
    SegmentExchangeOnSegments(
      ...
      () =>
        List
          .tabulate(pdptw.v)(vehicle =>
            vehicle -> ChainsHelper.computeCompleteSegments(pdptw, vehicle, oscarModel.chains)
          )
          .toMap,
      relevantNeighbors = () =>
        pdptw.kFirst(k, closestRelevantPredecessors, _ => (node: Int) => pdptw.isRouted(node)),
