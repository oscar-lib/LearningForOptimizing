...
class Model(val liLimProblem: LiLimProblem) {
  ...
  lazy val distanceAndTimeMatrix: Array[Array[Long]] =
    Array.tabulate(n)(from => {
      ...
      Array.tabulate(n)(to => {
        ...
        math
          .sqrt(
            Math.pow(fromPosition._1 - toPosition._1, 2) + Math
              .pow(fromPosition._2 - toPosition._2, 2)
          )
          .ceil
          .toLong
      })
  ...
  val routeLengthsInvariant: Array[CBLSIntVar] =
    RouteLength(pdpProblem.routes, n, v, (from, to) => distanceAndTimeMatrix(from)(to))
  ...
  val timeWindowViolations: Array[CBLSIntVar] =
    Array.tabulate(v)(vehicle =>
      CBLSIntVar(
        pdpProblem.m,
        0,
        Domain.coupleToDomain(0, 1),
        s"TimeWindow constraint on vehicle $vehicle"
      )
    )
  ...
  val contentVariationAtNode: Array[Long] =
    Array.fill(v)(0L) ++ liLimProblem.nodes.map(_.quantity).toArray
  // Constraint
  ...
  val vehicleCapacityViolations: Array[CBLSIntVar] =
    Array.tabulate(v)(vehicle =>
      CBLSIntVar(pdpProblem.m, 0, name = s"Capacity constraint on vehicle $vehicle")
    )
  // The constraint maintaining the above invariant
  val vehicleCapacityConstraint: GlobalVehicleCapacityConstraint =
    new GlobalVehicleCapacityConstraint(
      ...
      vehicleCapacityViolations
    )

  //////////// Precedences ////////////

  // Precedences data
  val precedences: List[(Int, Int)] =
    liLimProblem.demands.map(d => (d.fromNodeId + v - 1, d.toNodeId + v - 1))
  ...
  private def generateObjectiveFunction(vrp: VRP): Objective = {
    ...
    val obj = CascadingObjective(
      ...
      setSum(vrp.unrouted, x => 1) * unroutedNodePenalty + setSum(
        movingVehiclesInvariant,
        x => 1
      ) * usedVehiclePenalty + sum(routeLengthsInvariant)
    )
    vrp.m.close()
    obj
  }
