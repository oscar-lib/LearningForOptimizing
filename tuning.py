...
def run(trial: optuna.Trial):
    eps_start = trial.suggest_float("epsilon_start", 0.5, 1.0, log=True)
    eps_end = trial.suggest_float("epsilon_end", 0.0, 0.2)
    timeout = 450
    args = MultipleArgs(
        ...
        args={
            ...
            "epsilonEnd": eps_end,
            "epsilonNSecs": trial.suggest_int("epsilon_n_secs", 30, timeout, step=5),
...
if __name__ == "__main__":
    ...
    study = optuna.create_study(
        direction="minimize", study_name="CSP-r2-no_target", storage="sqlite:///tuning.db", load_if_exists=True
