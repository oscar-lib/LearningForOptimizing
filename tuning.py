...
def run_dqn(trial: optuna.Trial):
...
def run_ppo(trial: optuna.Trial):
    c1_start = trial.suggest_float("c1_start", 0.0, 1.0, step=0.01)
    c1_end = trial.suggest_float("c1_end", 0.0, c1_start, step=0.01)
    c2_start = trial.suggest_float("c2_start", 0.0, 1.0, step=0.01)
    c2_end = trial.suggest_float("c2_end", 0.0, 1.0, step=0.01)
    ...
    args = MultipleArgs(
        ...
        args={
            ...
            "batchSize": trial.suggest_int("batchSize", 5, memory_size, step=10),
            "clipping": trial.suggest_float("clipping", 0.0, 50.0),
            "memorySize": memory_size,
            "c1Start": c1_start,
            "c1End": c1_end,
            "c1NSecs": trial.suggest_int("epsilon_n_secs", 0, timeout, step=5),
            "c2Start": c2_start,
            "c2End": c2_end,
            "c2NSecs": trial.suggest_int("c2_n_secs", 0, timeout, step=5),
...
if __name__ == "__main__":
    ...
    study.optimize(run_ppo, n_trials=100)
